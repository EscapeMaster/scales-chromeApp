...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _template = __webpack_require__(120);

	var _template2 = _interopRequireDefault(_template);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	var client = new _connect.Client();
	...
	new _vue2.default({
	  ...
	  methods: {
	    reload: function reload() {
	      ...
	      client.send('reconnect', true).then(function () {
	     ...
	     * 列出用户选择的 USB 设备的信息
	     */
	    chooseUSB: function chooseUSB() {
	      var _this2 = this;

	      chrome.usb.getUserSelectedDevices({ multiple: true }, function (selected_devices) {
	        if (Array.isArray(selected_devices)) {
	          _this2.usbDevices = selected_devices;
	        }
	      });
	    }
	  },
	  created: function created() {
	    ...
	    client.on('all devices', function (devices) {
	      console.log('收到服务端发送过来的设备列表：', devices);
	      _this3.devices = devices;
	    });

	    client.on('device error', function (errorDevice) {
	      console.log('收到服务端发送过来的设备错误事件：', errorDevice);
	      var id = errorDevice.id;

	      _this3.devices.some(function (device) {
	        if (device.id === id) {
	    ...
	    client.on('data change', function (_ref) {
	      var newData = _ref.newData,
	          dataChangedDevice = _ref.device;

	      console.log('收到服务端发送过来的数据变化事件：', newData, dataChangedDevice);

	      var id = dataChangedDevice.id;


	      _this3.devices.some(function (device /*, i , a */) {
	        if (device.id === id) {
	          device.data = newData;
	          return true;
	        }
	    ...
	    client.on('device added', function (_ref2) {
	      var device = _ref2.device;
	      var id = device.id;

	      var has = _this3.devices.some(function (d, i, a) {
	        if (d.id === id) {
	          a.splice(i, 1, device);
	          return true;
	        }
	    ...
	    client.on('device removed', function (_ref3) {
	      var device = _ref3.device;
	      var id = device.id;

	      _this3.devices.some(function (d) {
	        if (d.id === id) {
	          d.error = '已断开';
	          return true;
	        }
